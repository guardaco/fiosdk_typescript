...
class GetObtData extends Query_1.Query {
    ...
    seenFilter(data, seen) {
        let result = data.obt_data_records;
        result = result.filter((record) => (!seen.find((seenOne) => seenOne.fio_request_id === record.fio_request_id && seenOne.payee_fio_public_key === record.payee_fio_public_key && seenOne.status === record.status)));
        return Object.assign(Object.assign({}, data), { obt_data_records: result });
    }
    decrypt(result) {
